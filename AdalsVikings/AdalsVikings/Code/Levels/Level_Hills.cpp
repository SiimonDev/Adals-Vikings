...
{
	if (Act1Events::hasBeenTriggered(Act1Event::ScaryWolf) && !Act1Events::hasBeenHandled(Act1Event::ScaryWolf))
	{
		if (!mFadeWolf1)
		{
			...
			if (FadeI.getFaded())
			{
				if (!FadeI.getWait())
				{
					AudioPlayer::playSound(Sound::Wolf, "Wolf", false);
					FadeI.setWaitDuration(sf::seconds(3));
				}
		...
		if (mFadeWolf1 && !mFadeWolf2)
		{	
			FadeI.fadeIn(frametime);
			if (FadeI.getFaded())
			{
				DialogHandler::startDialogue("Ulfr_Wolf_hills");
				mFadeWolf2 = true;
			}
		}
		if (DialogHandler::getDialogue("Ulfr_Wolf_hills").getHasStopped())
		{
			Act1Events::handleEvent(Act1Event::ScaryWolf);
		}
	}
...
{
	if (!Act1Events::hasBeenHandled(Act1Event::ScaryWolf) && mPlayer->getPosition().y <= 700 && !mPlayer->hasItemInInventory("sleepingMeat") && !Act1Events::hasBeenTriggered(Act1Event::GivenSleepingMeatToWolf))
	{
		Act1Events::triggerEvent(Act1Event::ScaryWolf);
	}
}
